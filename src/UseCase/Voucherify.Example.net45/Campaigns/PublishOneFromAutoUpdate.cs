...
{
    public class PublishOneFromAutoUpdate : IUseCase
    {
        public async Task Run(Voucherify.Api api)
        {
            try
            {
                ...
                Console.WriteLine("[Vouchers][PublishOneFromAutoUpdate] Published - Publication: {0}", voucherPublish);
                Console.ForegroundColor = ConsoleColor.DarkGreen;
                Console.WriteLine("[Campaigns][PublishOneFromAutoUpdate] Ok");
                Console.ResetColor();
            }
            catch (Exception e)
            {
                Console.ForegroundColor = ConsoleColor.DarkRed;
                Console.WriteLine("[Campaigns][PublishOneFromAutoUpdate] SomethingWentWrong: {0}", e);
                Console.ResetColor();
            }
        }
    }
